...
srs.CSRS = 63;                  % Bandwidth configuration C_SRS (0...63). It controls the allocated bandwidth to the SRS. Table 6.4.1.4.3-1: SRS bandwidth configuration.
...
srs.KTC = 2;                    % Comb number (2,4). Frequency density in subcarriers. 2 - every second subcarrier.
...
% Longer in-time domain PRB grid and SRS repetition of SRSPeriod(1) slots
numSlots = 10 * srs.SRSPeriod(1); % Display multiple periods for better visualization
L = 14; % Number of OFDM symbols per slot

% Initialize the extended resource grid
resourceGridLong = complex(zeros(carrier.NSizeGrid * 12, L * numSlots, srs.NumSRSPorts));

% Populate the extended resource grid with SRS symbols in the appropriate slots
for slot = 0:numSlots-1
    carrier.NSlot = slot;
    [srsIndices, ~] = nrSRSIndices(carrier, srs);
    srsSymbols = nrSRS(carrier, srs);
    
    % Adjust indices for the current slot
    [rowInd, colInd] = ind2sub([carrier.NSizeGrid * 12, L], srsIndices);
    colInd = colInd + slot * L;
    linearInd = sub2ind(size(resourceGridLong(:,:,1)), rowInd, colInd);
    
    % Populate the extended resource grid
    resourceGridLong(linearInd) = srsSymbols;
end

% Plot the extended resource grid
figure;
symbols = 0:(numSlots * L - 1);
subcarriers = 1:carrier.NSizeGrid * 12;
imagesc(symbols, subcarriers, abs(resourceGridLong(:,:,1)));
xlabel('OFDM symbol');
ylabel('Subcarrier');
axis xy;
title(sprintf('Longer In-Time Domain PRB Grid with SRS Repetition of %d slots', srs.SRSPeriod(1)));
colormap(parula);
colorbar;
