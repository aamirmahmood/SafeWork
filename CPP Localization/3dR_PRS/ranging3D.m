...
scenario = '3GPP_38.901_InF_LOS';   % Channel model

%%%%%%%%%%%% tx BSs positions %%%%%%%%%%%%
tx_position( : ,1) = [20; 120; 15];       % Position of BS-1 at (x; y; height) in meters
tx_position( : ,2) = [120; 20; 10];       % Position of BS-2 at (x; y; height) in meters
tx_position( : ,3) = [120; 120; 18];       % Position of BS-3 at (x; y; height) in meters

rx_mov = [2.5; 5; 1];     %RX movement in (x; y; speed)

for pos = 0:5

    rx_position = [40 + (rx_mov(1) *pos); 60 + (rx_mov(2) * pos); 1];       % Position of the receiver at (x; y; height) in meters
    ...
    for trial_cnt = 1:8
        ...
        [channel_freq_response, ~] = CIRs(tx_position, rx_position, scenario, rx_mov(3));                  % Channel impulse response modeled in QuaDriGa of size 3276xN; N - no. of gNbs's
    ...
    %     figure(1)
    %         subplot(3,2,1); plot(abs(PRS_freq_tx)); xlabel('subcarrier index [n]'); ylabel('amplitude PRS_{tx}');
    %         subplot(3,2,2); plot(angle(PRS_freq_tx)); xlabel('subcarrier index [n]'); ylabel('angle PRS_{tx}');
    %         subplot(3,2,3); plot(abs(PRS_freq_rx)); xlabel('subcarrier index [n]'); ylabel('amplitude PRS_{rx}');
    %         subplot(3,2,4); plot(angle(PRS_freq_rx)); xlabel('subcarrier index [n]'); ylabel('angle PRS_{rx}');
     ...
     for j=1 : size(tx_position, 2)
        ...
        PRS_freq_nz_rx(j,:) = angle(PRS_freq_nz_rx(j,:)) - angle(PRS_freq_nz_tx);
        
        %% Differential phase measurements 
        %PRS_delta_phi_1 = [];
        % Multicase to keep the phase continuous 
        i=1;
        for n=2:size(PRS_freq_nz_rx, 2)
            if ((PRS_freq_nz_rx(j, n) - PRS_freq_nz_rx(j, n-1)) > pi)
                PRS_delta_phi_1(j, i) = abs((2*pi) - (PRS_freq_nz_rx(j, n) - PRS_freq_nz_rx(j, n-1)));
    
            elseif ((PRS_freq_nz_rx(j, n) - PRS_freq_nz_rx(j, n-1)) < -pi)
                PRS_delta_phi_1(j, i) = abs((2*pi) + (PRS_freq_nz_rx(j, n) - PRS_freq_nz_rx(j, n-1)));
    
            elseif ((PRS_freq_nz_rx(j, n) - PRS_freq_nz_rx(j, n-1)) < 0)
               PRS_delta_phi_1(j, i) = abs(-(PRS_freq_nz_rx(j, n) - PRS_freq_nz_rx(j, n-1)));
       ...
       % PRS_delta_phi_m2 = [];
        i=1;
        for n=1:size(PRS_freq_nz_rx, 2)/2
            if ((PRS_freq_nz_rx(j, ((size(PRS_freq_nz_rx, 2)/2) + n)) - PRS_freq_nz_rx(j,n)) > pi)
                PRS_delta_phi_m2(j,i) = abs((2*pi) - (PRS_freq_nz_rx(j, (size(PRS_freq_nz_rx, 2)/2) + n) - PRS_freq_nz_rx(j,n)));
    
            elseif ((PRS_freq_nz_rx(j, ((size(PRS_freq_nz_rx, 2)/2) + n)) - PRS_freq_nz_rx(j,n)) < -pi)
                PRS_delta_phi_m2(j,i) = abs((2*pi) + (PRS_freq_nz_rx(j, (size(PRS_freq_nz_rx, 2)/2) + n) - PRS_freq_nz_rx(j,n)));
    
            elseif ((PRS_freq_nz_rx(j, ((size(PRS_freq_nz_rx, 2)/2) + n)) - PRS_freq_nz_rx(j,n)) < 0)
               PRS_delta_phi_m2(j,i) = abs(-((PRS_freq_nz_rx(j, (size(PRS_freq_nz_rx, 2)/2) + n)) - PRS_freq_nz_rx(j,n)));
        ...
        del_N_m2(j) = floor((size(PRS_freq_nz_rx, 2)/2) * delta_phi_1(j) - delta_phi_m2(j));
    ...
    %     figure(1) 
    %         subplot(3,2,5); plot(PRS_delta_phi_1); xlabel('subcarrier index [n]'); ylabel('angle \Delta\phi_{1} [rad]');
    %         subplot(3,2,6); plot(PRS_delta_phi_m2); xlabel('subcarrier index [n]'); ylabel('angle \Delta\phi_{m/2} [rad]');
    %     pause
